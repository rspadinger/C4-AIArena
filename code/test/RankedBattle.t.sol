...
contract RankedBattleTest is Test {
    ...
    function setUp() public {
        ...
        _rankedBattleContract = new RankedBattle(_ownerAddress, _GAME_SERVER_ADDRESS, address(_fighterFarmContract), address(_voltageManagerContract));
    ...
    function testClaimNRNRS() public {
        address player = vm.addr(3);
        _mintFromMergingPool(player);
        _fundUserWith4kNeuronByTreasury(player);
        vm.prank(player);
        ...
        _rankedBattleContract.updateBattleRecord(0, 50, 2, 1500, true); //player with tokenId=0 loses battle

        _rankedBattleContract.updateBattleRecord(1, 50, 0, 1500, true); //player2 with tokenId=1 loses battle
        vm.stopPrank();

        _rankedBattleContract.setNewRound();

        console.log("Points player1: ", _rankedBattleContract.accumulatedPointsPerAddress(player, 0));
        console.log("Points player2: ", _rankedBattleContract.accumulatedPointsPerAddress(player2, 0));

        console.log("Total points: ", _rankedBattleContract.totalAccumulatedPoints(0));

        //assertEq(_rankedBattleContract.accumulatedPointsPerAddress(player, 0), 40500); //this fails!
    }

    function test2ClaimNRNRS2() public {
        address staker = vm.addr(3);
        _mintFromMergingPool(staker);
        _fundUserWith4kNeuronByTreasury(staker);
        vm.prank(staker);
        ...
        vm.startPrank(address(_GAME_SERVER_ADDRESS));
        //uint256 tokenId, uint256 mergingPortion, uint8 battleResult, uint256 eloFactor, bool initiatorBool
        _rankedBattleContract.updateBattleRecord(0, 50, 0, 1500, true); //token0
        _rankedBattleContract.updateBattleRecord(0, 50, 0, 1500, true); //token0
        //vm.prank(address(_GAME_SERVER_ADDRESS));
        _rankedBattleContract.updateBattleRecord(1, 50, 0, 1500, true); //token1
        vm.stopPrank();
        ...
        // _rankedBattleContract.claimNRN();
        // assertEq(_rankedBattleContract.amountClaimed(staker) > 0, true);
        // console.log("Claimed: ", _rankedBattleContract.amountClaimed(staker));
    ...
    function testGetCurrentStakingData() public {
        ...
        vm.prank(address(_GAME_SERVER_ADDRESS));
        _rankedBattleContract.updateBattleRecord(0, 50, 0, 1500, true);
        (uint256 roundId, uint256 rankedNrnDistribution, uint256 totalAccumulatedPoints) = _rankedBattleContract.getCurrentStakingData();
        assertEq(roundId, 0);
