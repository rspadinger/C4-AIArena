...
contract RankedBattle {
    ...
    //@audit-ok can never be changed => make configurable
    ...
    constructor(address ownerAddress, address gameServerAddress, address fighterFarmAddress, address voltageManagerAddress) {
    ...
    function setStakeAtRiskAddress(address stakeAtRiskAddress) external {
        require(msg.sender == _ownerAddress);
        //@audit-ok we have 2: addr & instance
    ...
    function setNewRound() external {
        ...
        console.log("MP: ", address(_mergingPoolInstance));
        console.log("SAR: ", address(_stakeAtRiskInstance));
        //require(isAdmin[msg.sender]);
        require(msg.sender == address(_mergingPoolInstance), "Can only be called by the MergingPool");

        //@audit-ok use additional requires to make sure, the round is finished - how to determine if round is finished
        //require(totalAccumulatedPoints[roundId] > 0, "no points");

        _totalBattlesAlreadyIncremented = false;
        roundId += 1;
        //@audit-ok check if there are other contracts where we need to set the roundId
        _stakeAtRiskInstance.setNewRound(roundId);
        //@audit-ok in case the admin does not call setRankedNrnDistribution ?
    ...
    function stakeNRN(uint256 amount, uint256 tokenId) external {
        ...
        //@audit-ok could also be approver
    ...
    function updateBattleRecord(uint256 tokenId, uint256 mergingPortion, uint8 battleResult, uint256 eloFactor, bool initiatorBool) external {
        //@audit-ok track if this was already called for round & tokenId => what if this is called several times
        ...
        updateBattleRecordAlreadyCalled[roundId][tokenId] = true;

        //@audit-ok why is this increased for each call
        //totalBattles += 1;
    ...
    function getBattleRecord(uint256 tokenId) external view returns (uint32, uint32, uint32) {
        return (fighterBattleRecord[tokenId].wins, fighterBattleRecord[tokenId].ties, fighterBattleRecord[tokenId].loses);
    }
    ...
    function _addResultPoints(uint8 battleResult, uint256 tokenId, uint256 eloFactor, uint256 mergingPortion, address fighterOwner) private {
        uint256 stakeAtRisk;
        ...
        if (battleResult == 0) {
            ...
            //@audit-ok dont call if mergingPoints = 0
            ...
            if (accumulatedPointsPerFighter[tokenId][roundId] > 0) {
                ...
                //@audit-ok underflow: eg: first round, called first for loser
                //winner of a battle :: totalAccumulatedPoints[roundId] += points;
                totalAccumulatedPoints[roundId] -= points;
                if (points > 0) {
                ...
                //@audit-ok why do we need to transfer from this contract to _stakeAtRiskAddress
