...
contract FighterFarm is ERC721, ERC721Enumerable {
    ...
    uint8[2] public maxRerollsAllowed = [3, 3]; // 2 values: for Dendroids & normal fighters
    ...
    constructor(address ownerAddress, address delegatedAddress, address treasuryAddress_) ERC721("AI Arena Fighter", "FTR") {
    ...
    function incrementGeneration(uint8 fighterType) external returns (uint8) {
        ...
        maxRerollsAllowed[fighterType] += 1; //@audit-ok why => if someone enters the game at gen3, he has 6 rerolls within gen3, older players have only 1
    ...
    function claimFighters(uint8[2] calldata numToMint, bytes calldata signature, string[] calldata modelHashes, string[] calldata modelTypes) external {
        ...
        //@audit-ok dont use ecrecover => use OZ : sig malleability
    ...
    function updateFighterStaking(uint256 tokenId, bool stakingStatus) external {
        //@audit-ok who gets the staker role ? ==>> done by RankedBattle; stake & unstake
    ...
    function updateModel(uint256 tokenId, string calldata modelHash, string calldata modelType) external {
        //@audit-ok are we allowed to add any model => could I steel the values for performant fighters?
    ...
    function mintFromMergingPool(address to, string calldata modelHash, string calldata modelType, uint256[2] calldata customAttributes) public {
        ...
        _createNewFighter(
            ...
            modelHash,
            modelType,
            0, //@audit-ok only champions can be created, no Dendroids
            0, //icons type
            customAttributes
        );
    }
    ...
    function reRoll(uint8 tokenId, uint8 fighterType) public {
        ...
        _neuronInstance.approveSpender(msg.sender, rerollCost); //approves FighterFarm to spend  rerollCost on behalf of user

        bool success = _neuronInstance.transferFrom(msg.sender, treasuryAddress, rerollCost);

        //@audit-ok revert if !success => roll back the approval => check what happens if we call approveSpender several times
        if (success) {
            numRerolls[tokenId] += 1;
            uint256 dna = uint256(keccak256(abi.encode(msg.sender, tokenId, numRerolls[tokenId])));
            (uint256 element, uint256 weight, uint256 newDna) = _createFighterBase(dna, fighterType);

            //@audit-ok do we have all atttribs ? modelHash, modelType, iconsType dont change
    ...
    function getAllFighterInfo(uint256 tokenId) public view returns (address, uint256[6] memory, uint256, uint256, string memory, string memory, uint16) {
    ...
    function _createNewFighter(
        ...
        //@audit-ok check attr
        fighters.push(FighterOps.Fighter(weight, element, attrs, newId, modelHash, modelType, generation[fighterType], iconsType, dendroidBool));
