...
contract AAMintPass is ERC721, ERC721Burnable {
    ...
    function claimMintPass(uint8[2] calldata numToMint, bytes calldata signature, string[] calldata _tokenURIs) external {
        require(!mintingPaused);
        bytes32 msgHash = bytes32(keccak256(abi.encode(msg.sender, numToMint[0], numToMint[1], passesClaimed[msg.sender][0], passesClaimed[msg.sender][1], _tokenURIs)));
